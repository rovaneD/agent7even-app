...
| V5 | May 2026 | Notification system (bell, notification center, real-time, 6 event triggers). Design & Development + Packaging inquiry flow (scope-required services). Team management system (invite, permissions, seat billing). Marketing site service pages (index + 9 individual pages + shared layout). LogoStrip. Dashboard PlanBanner (dismissible, correct plan labels). Analytics renamed Website → Google Analytics with Property ID subtitle. Buffer API confirmed dead end — use Later/Publer instead. |
...
| `sort_order` | int | |
| `is_active` | boolean | Toggleable from Admin Settings |
| `category` | text | e.g. `design_dev`, `packaging` — used to identify scope-required services |
| `requires_scope` | boolean | If true, service card routes to inquiry form instead of order modal |
| `created_at` | timestamptz | |

Seeded with 10 services: Social Media Management, Website Building, Brand Identity & Logo, Email Marketing Setup, Product Photography, SEO Basics, Video & Reels Editing, Ad Management, Design & Development (`requires_scope: true`), Packaging Design (`requires_scope: true`).
...
- [x] **Marketing service pages** — `/services` index + 9 individual service pages + `ServicePageLayout` shared component. Visual pages with sample slots: Website Building, Social Media, Brand Identity, Video & Reels, Packaging, Design & Development. Detailed pages: Email Marketing, Product Photography, SEO, Ad Management.
- [x] **Packaging Design service** — added to `services` table with `requires_scope: true`, routes to inquiry form
...
- **Later or Publer** — Social scheduling with proper OAuth API. Buffer NOT viable: stopped accepting new developer OAuth registrations as of 2026 — cannot get a client_id. Use Later or Publer instead. For clients already on Buffer, Zapier bridges the gap.
